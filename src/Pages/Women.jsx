...
const Women = () => {
  ...
  const fetchWomenProducts = async () => {
    try {
      const docRef = doc(db, "products", "Woman");
      const docSnap = await getDoc(docRef);

      if (docSnap.exists()) {
        const data = docSnap.data();
        console.log("Fetched Women data:", data);
        setProducts(data.products || []);
      } else {
        console.warn("Women document not found in Firestore.");
      }
    } catch (error) {
        ...
        {products.map((product) => (
          ...
          >
            <div className="product-image-wrapper">
              <span className="tag-new">Women</span>
              <img
